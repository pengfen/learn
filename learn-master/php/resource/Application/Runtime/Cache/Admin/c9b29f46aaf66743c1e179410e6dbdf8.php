...
<div id="add">
    <a href="javascript:void(0)">添加项目记录</a>
    <input type='hidden' value="<?php echo U('Project/add');?>"/>
    <a href="<?php echo U('Project/index');?>">返回项目记录列表</a>
</div>
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr style="height:35px; line-height: 35px; overflow:hidden;">
	    		...
	    		<td>
	    			...
	    			<input type="hidden" value="<?php echo U('Project/detail', array('id'=>$v[id]));?>">
	    		</td>
	    		<td><?php echo date('Y-m-d H:i:s', $info[starttime]);?></td>
	    		<td><?php echo date('Y-m-d H:i:s', $info[endtime]);?></td>
	    		<td>
	    			<a class="detail" href="javascript:void(0)">详情</a>
	    			<input type="hidden" value="<?php echo U('Project/detail', array('id'=>$v[id]));?>">
	    			<a href="<?php echo U('Dir/index', array('pid'=>$v[id],'name'=>$v[name]));?>">后台目录列表</a>
	    			<!-- <a href="javascript:void(0)">前台目录列表</a> -->
	    			<a href="<?php echo U('Assoc/index', array('pid'=>$v[id]));?>">目录关联列表</a>
	    			<a class="upd" href="javascript:void(0);">修改</a>
				  	<input type="hidden" value="<?php echo U('Project/edit', array('id'=>$v[id]));?>"/>
				  	<a href="<?php echo U('Project/delete', array('id'=>$v[id]));?>">删除</a>
	    		</td>
