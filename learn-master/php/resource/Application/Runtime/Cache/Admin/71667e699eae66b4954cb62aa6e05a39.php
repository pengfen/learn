...
<html lang="en">
    ...
    <div id="add">
        <?php if($_SESSION['manager']['id'] == '1'): ?><a href="javascript:void(0)">添加管理员</a>
	    <input type='hidden' value="<?php echo U('Manager/add');?>"/><?php endif; ?>
	    <a href="<?php echo U('Manager/index');?>">返回管理员列表</a>
	</div>
			...
			<table class='table table-striped'>
				...
				<tbody class="managerMain">
					<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php if($v['rid'] != '0'): echo ($v["rid"]); ?>
								<?php else: ?>超级管理员<?php endif; ?>
							</td>
							<td><?php echo date('Y-m-d H:i:s',$v['regtime']);?></td>
							<td><?php echo date('Y-m-d H:i:s',$v['logtime']);?></td>
							<td>
							    <?php if($_SESSION['manager']['id'] == $v['id']): ?><a class='upd' href="javascript:void(0);">修改</a> 
							    <input type='hidden' value="<?php echo U('Manager/edit', array('id'=>$v[id]));?>" /><?php endif; ?>
							    <?php if($v['id'] != '1' and $_SESSION['manager']['id'] == '1'): ?><a href="<?php echo U('Manager/delete?id='.$v['id']);?>">删除</a><?php endif; ?>
							</td>
