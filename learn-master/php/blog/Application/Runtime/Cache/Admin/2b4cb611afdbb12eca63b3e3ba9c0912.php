	...
	<table class="table table-striped">
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr>
	    		...
	    		<td><?php if($v[auth] == 0): ?>所有人可见<?php elseif($v[auth] == 1): ?>仅好友可见<?php elseif($v[auth] == 2): ?>指定好友可见<?php else: ?>仅自己可见<?php endif; ?></td>
	    		<td>
	    			<a class="upd" href="javascript:void(0);">查看详情</a>
				  	<input type="hidden" name="" value="<?php echo U('Log/detail', array('id'=>$v[id], 'name'=>$name, 'cate'=>$cate));?>"/>
				  	<?php if($v['flag'] == 1): ?><a href="javascript:void(0)">已删除</a>
				  	<?php else: ?>
				  	<a href="<?php echo U('Log/delete', array('id'=>$v[id], 'name'=>$name, 'cate'=>$cate));?>">删除</a><?php endif; ?>
	    		</td>
