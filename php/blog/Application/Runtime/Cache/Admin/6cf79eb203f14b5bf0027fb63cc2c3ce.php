...
<div id = 'right' class="col-md-10">
	<table class="table table-striped">
	    <thead>
	    	<tr>
	    		<th>编号</th>
	    		<?php if($name != ''): ?><th>用户名</th><?php endif; ?>
	    		<th>说说内容</th>
	    		<th>添加时间</th>
	    		<th>状态</th>
	    		<th>操作</th>
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr>
	    		<td><?php echo ($v["id"]); ?></td>
	    		<?php if($name != ''): ?><td><?php echo ($name); ?></td><?php endif; ?>
	    		<td><?php echo ($v["content"]); ?></td>
	    		<td><?php echo date('Y-m-d H:i:s', $v[addtime]);?></td>
	    		<td><?php if($v[state] == 0): ?>启用<?php else: ?>禁用<?php endif; ?></td>
	    		<td>
	    			<a class="update" href="javascript:void(0)">修改</a>
	    			<input type="hidden" name="" value="<?php echo U('User/edit', array('id'=>$v[id], 'state'=>$v['state'], 'name'=>$name));?>" />
	    		</td>
