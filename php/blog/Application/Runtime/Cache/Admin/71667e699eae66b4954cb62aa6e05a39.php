...
<html lang="en">
	...
	<div class='admin-mess'><?php echo ($_GET['mess']); ?></div>
			<div class='col-md-10' id = 'right'>
				<h2 class="sub-header"><i>后台用户管理</i></h2>
				<a href="<?php echo U('Manager/add');?>">添加管理员</a>
				<b><a href="<?php echo U('Manager/index');?>" style='color:#87B87F;float:right;margin-right:60px'>查全部</a></b>
						...
						<table class='table table-striped'>
							...
							<tbody class="managerMain">
								<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										...
										<td>
											<?php if(($vo['rid'] == 1)): ?>主管
											<?php elseif(($vo['rid'] == 2)): ?>经理
											<?php elseif(($vo['rid'] == 3)): ?>超级管理员
											<?php elseif(($vo['rid'] == 4)): ?>职员<?php endif; ?>
										</td>
										<td><?php echo date('Y-m-d H:i:s',$vo['regtime']);?></td>
										<td><?php echo date('Y-m-d H:i:s',$vo['logtime']);?></td>
										<td><a href="<?php echo U('Manager/edit?id='.$vo['id']);?>">编辑</a> <a href="<?php echo U('Manager/delete?id='.$vo['id']);?>">删除</a></td>
