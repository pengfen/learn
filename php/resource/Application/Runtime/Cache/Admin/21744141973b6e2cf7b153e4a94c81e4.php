...
<div id="add">
    <a href="javascript:void(0)">添加课程列表</a>
    <input type='hidden' value="<?php echo U('Lesson/add');?>"/>
    <a href="<?php echo U('Lesson/index');?>">返回课程列表</a>
</div>
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr style="height:35px; line-height: 35px; overflow:hidden;">
	    		...
	    		<td>
	    			...
	    			<input type="hidden" value="<?php echo U('Lesson/detail', array('id'=>$v[id]));?>">
	    		</td>
	    		...
	    		<td>
	    			...
	    			<a class="upd" href="javascript:void(0);">修改</a>
				  	<input type="hidden" value="<?php echo U('Lesson/edit', array('id'=>$v[id]));?>"/>
				  	<a class="upd-video" href="javascript:void(0);">修改课程视频</a>
				  	<input type="hidden" value="<?php echo U('Lesson/video', array('id'=>$v[id]));?>"/>
				  	<a class="upd-data" href="javascript:void(0);">修改课程相关资料</a>
				  	<input type="hidden" value="<?php echo U('Lesson/data', array('id'=>$v[id]));?>"/>
				  	<a class="upd-img" href="javascript:void(0);">修改流程图</a>
				  	<input type="hidden" value="<?php echo U('Lesson/img', array('id'=>$v[id]));?>"/>
				  	<a href="<?php echo U('Lesson/delete', array('id'=>$v[id]));?>">删除</a>
	    		</td>
...
<script type="text/javascript">
	...
	click($('.upd-video'));    // 动态加载修改课程视频界面模板
	click($('.upd-data'));    // 动态加载修改课程相关资料界面模板
	click($('.upd-img'));    // 动态加载修改流程图界面模板
