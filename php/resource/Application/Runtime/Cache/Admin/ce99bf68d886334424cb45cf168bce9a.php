...
<div id="add">
    <a href="javascript:void(0)">添加实现记录</a>
    <input type='hidden' value="<?php echo U('Realize/add');?>"/>
    <a href="<?php echo U('Realize/index');?>">返回实现记录</a>
</div>
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr style="height:35px; line-height: 35px; overflow:hidden;">
	    		...
	    		<td>
	    			...
	    			<input type="hidden" value="<?php echo U('Realize/detail', array('id'=>$v[id]));?>">
	    		</td>
	    		...
	    		<td>
	    			...
	    			<a class="upd" href="javascript:void(0);">修改</a>
				  	<input type="hidden" name="" value="<?php echo U('Realize/edit', array('id'=>$v[id]));?>"/>
				  	<a href="<?php echo U('Realize/delete', array('id'=>$v[id]));?>">删除</a>
	    		</td>
