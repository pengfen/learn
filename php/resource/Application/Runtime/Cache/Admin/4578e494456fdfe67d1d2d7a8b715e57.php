...
<div id="add">
    <a href="javascript:void(0)">添加关联页</a>
    <input type='hidden' value="<?php echo U('AssocPage/add');?>"/>
    <a href="<?php echo U('AssocPage/index');?>">返回关联页列表</a>
</div>
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr style="height:35px; line-height: 35px; overflow:hidden;">
	    		...
	    		<td>
	    			...
	    			<input type="hidden" value="<?php echo U('AssocPage/detail', array('id'=>$v[id]));?>">
	    		</td>
	    		...
	    		<td>
	    			...
	    			<a class="upd" href="javascript:void(0);">修改</a>
				  	<input type="hidden" name="" value="<?php echo U('AssocPage/edit', array('id'=>$v[id]));?>"/>
				  	<a href="<?php echo U('AssocPage/delete', array('id'=>$v[id]));?>">删除</a>
	    		</td>
