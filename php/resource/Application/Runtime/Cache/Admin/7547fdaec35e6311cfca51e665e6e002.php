...
<div id="add">
    <a href="javascript:void(0)">添加Bug</a>
    <input type='hidden' value="<?php echo U('Bug/add');?>"/>
    <a href="<?php echo U('Bug/index');?>">返回Bug 列表</a>
</div>
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr style="height:35px; line-height: 35px; overflow:hidden;">
	    		...
	    		<td>
	    			...
	    			<input type="hidden" value="<?php echo U('Bug/detail', array('id'=>$v[id]));?>">
	    		</td>
	    		...
	    		<td>
	    			...
	    			<a class="upd" href="javascript:void(0);">修改</a>
				  	<input type="hidden" name="" value="<?php echo U('Bug/edit', array('id'=>$v[id]));?>"/>
				  	<a class="upd-img" href="javascript:void(0);">修改bug</a>
				  	<input type="hidden" name="" value="<?php echo U('Bug/img', array('id'=>$v[id]));?>"/>
				  	<a href="<?php echo U('Bug/delete', array('id'=>$v[id]));?>">删除</a>
	    		</td>
...
<script type="text/javascript">
	...
	click($('.upd-img')); // 动态加载修改bug 后的界面模板
