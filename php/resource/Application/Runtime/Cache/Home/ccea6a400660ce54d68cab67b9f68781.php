...
<div class="w content">
    <div class='admin-success'>
		<?php
 if(!empty($_GET['sucno'])){ if($_GET['sucno'] == '11'){ echo '恭喜您 注册成功 请登录'; } if($_GET['sucno'] == '12'){ echo '恭喜您 修改密码成功 请登录'; } } ?>
	</div>
	...
	<form action="<?php echo U('Login/checkLogin');?>" method="post">
		...
		<a class='user-regist' href='./reg.php'>注册新账号</a>
		<button class="loginBtn">登&nbsp;录</button>
	</form>

	<div class='error-text'>
		<?php
 if(!empty($_GET['errno'])){ $errno = $_GET['errno']; if($errno == '12') echo '您没有登录请登录'; if($errno == '3') echo '密码长度不在指定范围内'; if($errno == '5') echo '密码错误'; if($errno == '8') echo '邮箱格式错误'; if($errno == '1') echo '用户名不对'; } ?>
	</div>
	</div>
